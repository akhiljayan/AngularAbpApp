...
(function () {
    ...
    appModule.directive('uiBreadcrumbs', ['$interpolate', '$state', '$stateParams', '$previousState', 'abp.services.app.breadcrumb', 'breadCrumbFact', '$filter',
        function ($interpolate, $state, $stateParams, $previousState, $breadcrumbService, breadCrumbFact, $filter) {
            return {
                restrict: 'E',
                templateUrl: '/App/common/directives/breadCrumbTemplate.cshtml',
                scope: {
                    displaynameProperty: '@',
                ...
                link: function (scope) {
                    ...
                    function updateBreadcrumbsArray() {
                        ...
                        while (!rootParent) {
                            if (currentState.breadcrumb.callFromService) {
                                var data = currentState.breadcrumb.displayName;
                                breadCrumbFact.getDisplayName(data).then(function (result) {
                                    var dataArr = $filter('filter')(scope.breadcrumbs, { 'route': result.route });
                                    dataArr.displayName = result.name;
                                    var index = scope.breadcrumbs.findIndex(function (x) {
                                        return x.route === result.route;
                                    });
                                    scope.breadcrumbs[index] = dataArr;
                                });
                            } else {
                                displayName = currentState.breadcrumb.displayName;
                            }
                            ...
                            var parent = $state.get(currentState.displayName.parent);
                            if (parent) {
                                currentState = parent;
                                rootParent = false;
                            } else {
