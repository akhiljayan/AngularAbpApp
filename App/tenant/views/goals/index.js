(function () {
    appModule.controller('tenant.views.goals.index', [
        '$scope', '$uibModal', '$stateParams', 'uiGridConstants', 'abp.services.app.goal',
        function ($scope, $uibModal, $stateParams, uiGridConstants, goalService) {
            var vm = this;
            vm.loading = false;
            ...
            vm.deleteSelectedGoal = function () {
                ...
                abp.message.confirm(
                    ...
                    function (isConfirmed) {
                        if (isConfirmed) {
                            ...
                            goalService.deleteSelectedGoal(
                                deletelist
                            ).then(function () {
                                vm.getAllGoals();
            ...
            vm.rowDblClick = function (row) {
                vm.editGoal(row.entity.id);
            };

            vm.editGoal = function (id) {
                openCreateOrEditModal(id);
            };

            function openCreateOrEditModal(id, action) {
                var modalInstance = $uibModal.open({
                    templateUrl: '~/App/tenant/views/Goals/createOrEditGoal.cshtml',
                    controller: 'tenant.views.goals.createOrEditGoal as vm',
                    backdrop: 'static',
                    resolve: {
                        id: function () {
                            return id;
