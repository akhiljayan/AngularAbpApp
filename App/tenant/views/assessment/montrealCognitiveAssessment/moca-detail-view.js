(function () {
    appModule.component('mocaDetailView', {
        bindings: {
            title: '<',
            mode: '<',
            template: '<',
        ...
        controller: [
            ...
            function ($scope, $uibModal, $timeout, commonFormFunction, commonLookupService, $stateParams, $state, $anchorScroll, $location, $filter, $rootScope, assessmentService, stateNavigation) {
                ...
                vm.$onInit = function () {
                    ...
                    vm.multiplechoicecue = 'Face: Nose, Face, Hand <br/>Silk: Denim, Cotton, Silk <br/>Church: Church, School, Hospital <br/>Rose: Daisy, Rose, Tulip <br/>Red: Red, Blue, Green';
                    vm.categorycue = 'Face: Part of the body <br/>Silk: Type of fabric <br/>Church: Type of building <br/>Rose: Type of flower <br/>Red: Type of colour';
                ...
                // Generate the chart and do checking when person is changed.
                            ...
                            for (var i = 0; i < vm.model.actualAssessment.answers.length; i++) {
                                ...
                                if (val.question.mutuallyExclusiveQuestionId != null) {
                                    ...
                                    if (val.question.questionType == vm.assessmentQuestionType.MultiSelectListing) {
                                        vm.model.selectedValueList[val.optionId] = JSON.stringify({ "id": val.optionId, "score": val.option.score, "questionId": val.option.questionId, "optionDescription": val.option.description, "questionType": val.question.questionType });
                                        vm.model.selectedValue[val.optionId] = true;

                                        /* Question with OpenOption -> Please specify the number of animals named. value handling.*/
                                        if (val.question.openOption) {
                        ...
                        assessmentService.createPersonAssessment(vm.model.actualAssessment).then(function (result) {
                            ...
                            if (closed) {
                                ...
                                if (typeof $stateParams.personId != 'undefined') {
                                    $state.go('tenant.mocaDetailPersonMode', { assessment: $stateParams.assessment, id: result.data.id, personId: $stateParams.personId });
                                } else {
                                    $state.go('tenant.mocaDetail', { assessment: $stateParams.assessment, id: result.data.id });
                                }
                        ...
                        if (vm.model.selectedValue.hasOwnProperty(key)) {
                            ...
                            /* Socre calculation for MultiSelect Checkbox Question Type. */
                                ...
                                if (val) {
                                    ...
                                    /* To get Please specify the number of animals named. value when the question is OpenOption*/
                                    if (val.optionDescription == 'N >= 11 words') {
                                        var otherPleaseSpecifyVal = vm.model.selectedTextValue[val.questionId];
                                        answer.answerTextValue = otherPleaseSpecifyVal;
                                    }
                ...
                vm.onChangeOption = function () {
                    ...
                    if (vm.model.isScoreCalculationRequired) {
                        ...
                        var differentScoringLogicOptionsCount = 0;
                        for (var key in vm.model.differentScoringLogicOptions) {
                            if (vm.model.differentScoringLogicOptions.hasOwnProperty(key) && vm.model.differentScoringLogicOptions[key]) {
                                differentScoringLogicOptionsCount += 1;
                            }
                        }
                        /* 
                         * To apply out of box score calculation logic for this specific question
                         * Question: Serial 7 subtraction starting at 100
                         * Score Logic: 4 or 5 correct subtractions: 3 pts, 2 or 3 correct: 2 pts, 1 correct: 1 pt, 0 correct: 0 pt
                         */
                        if (differentScoringLogicOptionsCount == 4 || differentScoringLogicOptionsCount == 5) {
                            vm.model.totalScore += 3;
                        } else if (differentScoringLogicOptionsCount == 2 || differentScoringLogicOptionsCount == 3) {
                            vm.model.totalScore += 2;
                        } else if (differentScoringLogicOptionsCount == 1) {
                            vm.model.totalScore += 1;
                        }
                    };
                    vm.model.actualAssessment.totalScore = vm.model.totalScore;
                };

                /* It's handling for MultiSelectListing (checkbox) Option Check.*/
                vm.model.differentScoringLogicOptions = [];
                vm.onCheckOption = function (option, question) {
                    
                    if (vm.model.selectedValue[option.id] == true) {
                        var jsonObject = { id: option.id, score: option.score, questionId: option.questionId, optionDescription: option.description };
                    ...
                    if (option.description == 'N >= 11 words' && vm.model.selectedValue[option.id] == true) {
                        vm.model.selectedOtherPleaseSpecifyValue[question.id] = true;
                    } else if (option.description == 'N >= 11 words' && vm.model.selectedValue[option.id] == false) {
                        vm.model.selectedOtherPleaseSpecifyValue[question.id] = false;
                        vm.model.selectedTextValue[question.id] = null;
                    }
                     ...
                     * Score Logic: 4 or 5 correct subtractions: 3 pts, 2 or 3 correct: 2 pts, 1 correct: 1 pt, 0 correct: 0 pt
                     */
                    if (question.title == 'Serial 7 subtraction starting at 100') {
                        if (vm.model.selectedValue[option.id] == true) {
                            vm.model.differentScoringLogicOptions[option.id] = true;
                        } else {
                            vm.model.differentScoringLogicOptions[option.id] = false;
                        }
                    }
                ...
                vm.bindToJson = function (option, mutualId, question) {
                    if (mutualId == null) {
                        vm.model.checkValue[option.questionId] = false;
                    } else {
                        vm.model.checkValue[mutualId] = false;
                    }

                    var jsonObject = { id: option.id, score: option.score, questionId: option.questionId, optionDescription: option.description, questionType: question.questionType };
                    return jsonObject;
                };
        ...
        templateUrl: '~/App/tenant/views/assessment/montrealCognitiveAssessment/moca-detail-view.cshtml'
    });
})();