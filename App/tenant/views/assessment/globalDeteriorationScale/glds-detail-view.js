(function () {
    appModule.component('gldsDetailView', {
        bindings: {
            title: '<',
            mode: '<',
            template: '<',
        ...
        controllerAs: 'vm',
        controller: [
            '$scope', '$uibModal', '$timeout', 'commonFormFunction', '$stateParams', '$state', '$anchorScroll', '$location', '$filter', '$rootScope', 'abp.services.app.familyMember', 'abp.services.app.globalDeteriorationScale', 'abp.services.app.assessment', 'stateNavigation',
            function ($scope, $uibModal, $timeout, commonFormFunction, $stateParams, $state, $anchorScroll, $location, $filter, $rootScope, familyMemberService, globalDeteriorationScaleService, assessmentService, stateNavigation) {
                ...
                vm.load = function () {
                    ...
                    if ($stateParams.id) {
                        ...
                        vm.mode = 'view';
                        globalDeteriorationScaleService.getPersonAssessmentById(vm.actualAssessmentId).then(function (result) {
                ...
                vm.save = function (closed) {
                    ...
                    convertAnswer();
                    if ($stateParams.id) {
                        globalDeteriorationScaleService.updatePersonAssessment(vm.model.actualAssessment).then(function () {
                        ...
                        globalDeteriorationScaleService.createPersonAssessment(vm.model.actualAssessment).then(function (result) {
                            abp.notify.info(app.localize('SavedSuccessfully'));
                            if (closed) {
                                vm.close();
                            } else {
                                if (typeof $stateParams.personId != 'undefined') {
                                    $state.go('tenant.gldsDetailPersonMode', { assessment: $stateParams.assessment, id: result.data.id, personId: $stateParams.personId });
                                } else {
                                    $state.go('tenant.gldsDetail', { assessment: $stateParams.assessment, id: result.data.id });
                                }
                ...
                vm.confirm = function () {
                    ...
                    if (vm.model.totalScore == 0) {
                        abp.message.warn('Please select a stage.');
                        vm.stageNotSelected = true;
                        return;
                    }
                        ...
                        abp.message.confirm(
                            ...
                            function (isConfirmed) {
                                if (isConfirmed) {
                                    convertAnswer(true);
                                    globalDeteriorationScaleService.updatePersonAssessment(vm.model.actualAssessment).then(function () {
                                        globalDeteriorationScaleService.confirm(vm.actualAssessmentId).then(function (result) {
                                            vm.load();
                ...
                vm.actualClone = function (title, message) {
                    abp.message.confirm(
                        ...
                        function (isCloned) {
                            if (isCloned) {
                                globalDeteriorationScaleService.clone(vm.model.actualAssessment.id).then(function (result) {
                                    if (typeof $stateParams.personId != 'undefined') {
                                        $state.go('tenant.gldsDetailPersonMode', { assessment: $stateParams.assessment, id: result.data.id, personId: $stateParams.personId });
                                    } else {
                                        $state.go('tenant.gldsDetail', { assessment: $stateParams.assessment, id: result.data.id });
                ...
                vm.delete = function () {
                    ...
                    abp.message.confirm(
                        ...
                        function (isConfirmed) {
                            if (isConfirmed) {
                                globalDeteriorationScaleService.deletePersonAssessment(
                ...
                function clearAllSelectedStage(option) {

                    for (var key in vm.model.selectedValueList) {
                        var val = JSON.parse(vm.model.selectedValueList[key]);
                        if (val != null && val.id != option.id) {
                            vm.model.selectedValueList[val.id] = null;
                        }
                    }
                    ...
                    for (var key in vm.model.selectedValue) {
                        ...
                        if (optionId != option.id) {
                            vm.model.selectedValue[optionId] = false;
                        }
                    }
                };               
        ...
        templateUrl: '~/App/tenant/views/assessment/globalDeteriorationScale/glds-detail-view.cshtml'
    });
})();