(function () {
    appModule.component('psDetailView', {
        bindings: {
            title: '<',
            mode: '<',
            template: '<',
        ...
        controllerAs: 'vm',
        controller: [
            '$scope', '$uibModal', '$timeout', 'commonFormFunction', 'abp.services.app.commonLookup', '$stateParams', '$state', '$anchorScroll', '$location', 'abp.services.app.user', '$filter', '$rootScope', 'abp.services.app.painAssessment', 'abp.services.app.assessment', 'stateNavigation',
            function ($scope, $uibModal, $timeout, commonFormFunction, commonLookupService, $stateParams, $state, $anchorScroll, $location, userService, $filter, $rootScope, painAssessmentService, assessmentService, stateNavigation) {
                ...
                vm.$doCheck = function () {                    
                    if (!angular.equals(vm.preTotalScore, vm.model.actualAssessment.score) && vm.model.actualAssessment.assessmentId != undefined) {                        
                        if (vm.model.actualAssessment.score >= 0 && vm.model.actualAssessment.score <= 10) {
                            if (vm.categories != undefined && vm.categories.length != 0) {
                                vm.option.barColor = getColorCode(vm.model.actualAssessment.score);
                            } else {
                            ...
                            vm.preTotalScore = vm.model.actualAssessment.score;
                            vm.model.totalScore = vm.model.actualAssessment.score;
                            vm.invalidScore = false;
                        } else {
                            vm.invalidScore = true;
                            vm.preTotalScore = null;
                        }
                    }
                };
                ...
                vm.load = function () {
                    ...
                    if ($stateParams.id) {
                        ...
                        vm.mode = 'view';
                        painAssessmentService.getPersonAssessmentById(vm.actualAssessmentId).then(function (result) {
                            ...
                            for (var i = 0; i < vm.model.actualAssessment.answers.length; i++) {
                                ...
                                if (val.question.mutuallyExclusiveQuestionId != null) {
                                    ...
                                    if (val.question.questionType == vm.assessmentQuestionType.SingleSelectListing) {
                                        ...
                                        vm.model.selectedValueList[val.optionId] = JSON.stringify({ "id": val.optionId, "score": val.option.score, "description": val.option.description, "questionId": val.option.questionId, "questionType": val.question.questionType });
                                        vm.model.selectedValue[val.optionId] = true;

                                        /* MultiSelectListing with OpenOption -> Others,Please Specify field value handling.*/
                                        ...
                                        vm.model.selectedValueList[val.questionId] = JSON.stringify({ "id": val.optionId, "score": val.option.score, "description": val.option.description, "questionId": val.option.questionId, "questionType": val.question.questionType });
                        ...
                        painAssessmentService.getPersonAssessmentById(vm.actualAssessmentId).then(function (result) {
                            ...
                            vm.model.actualAssessment.completedById = null;
                            vm.model.actualAssessment.receivedByPainManagementTeamId = null;
                ...
                vm.save = function (closed) {
                    ...
                    convertAnswer();
                    if ($stateParams.id) {
                        painAssessmentService.updatePersonAssessment(vm.model.actualAssessment).then(function () {
                        ...
                        painAssessmentService.createPersonAssessment(vm.model.actualAssessment).then(function (result) {
                            abp.notify.info(app.localize('SavedSuccessfully'));
                            if (closed) {
                                vm.close();
                            } else {
                                if (typeof $stateParams.personId != 'undefined') {
                                    $state.go('tenant.psDetailPersonMode', { assessment: $stateParams.assessment, id: result.data.id, personId: $stateParams.personId });
                                } else {
                                    $state.go('tenant.psDetail', { assessment: $stateParams.assessment, id: result.data.id });
                                }
                        ...
                        abp.message.confirm(
                            ...
                            function (isConfirmed) {
                                if (isConfirmed) {
                                    convertAnswer(true);
                                    painAssessmentService.updatePersonAssessment(vm.model.actualAssessment).then(function () {
                                        painAssessmentService.confirm(vm.actualAssessmentId).then(function (result) {
                                            vm.load();
                ...
                vm.actualClone = function (title, message) {
                    abp.message.confirm(
                        ...
                        function (isCloned) {
                            if (isCloned) {
                                painAssessmentService.clone(vm.model.actualAssessment.id).then(function (result) {
                                    if (typeof $stateParams.personId != 'undefined') {
                                        $state.go('tenant.psDetailPersonMode', { assessment: $stateParams.assessment, id: result.data.id, personId: $stateParams.personId });
                                    } else {
                                        $state.go('tenant.psDetail', { assessment: $stateParams.assessment, id: result.data.id });
                ...
                vm.delete = function () {
                    ...
                    abp.message.confirm(
                        ...
                        function (isConfirmed) {
                            if (isConfirmed) {
                                painAssessmentService.deletePersonAssessment(
                ...
                function convertAnswer(isConfirm) {                    
                    ...
                    for (var key in vm.model.selectedValue) {                        
                        ...
                        if (vm.model.selectedValue.hasOwnProperty(key)) {                           
                            ...
                            /* Converting answer for SingleSelectListing*/
                            ...
                            /* Converting answer for SingleSelectDropDown*/
                            ...
                            /* Converting answer for MultiSelect Checkbox. */
                    ...
                    for (var key in vm.model.selectedTextValue) {
                        ...
                        if (!vm.model.selectedValue.hasOwnProperty(key)) {
                            if (vm.model.selectedTextValue.hasOwnProperty(key) && !vm.model.selectedOtherPleaseSpecifyValue.hasOwnProperty(key)) {
                ...
                vm.onCheckOption = function (option, question) {                    
                    if (vm.model.selectedValue[option.id] == true) {
                        var jsonObject = { id: option.id, score: option.score, description: option.description, questionId: option.questionId };
                        vm.model.selectedValueList[option.id] = JSON.stringify(jsonObject);

                        /* To show/hide Others,Please Specify field.*/
                        if (question.openOption) {
                            if (option.description == 'Others') {
                                vm.model.selectedOtherPleaseSpecifyValue[option.questionId] = true;
                            }
                        }
                        ...
                        if (question.openOption) {
                            if (option.description == 'Others') {
                                vm.model.selectedOtherPleaseSpecifyValue[option.questionId] = false;
                                vm.model.selectedTextValue[option.questionId] = null;
                            }
                        }
                    }
                }
                ...
                vm.onChangeDropDownSingleSelect = function (selected, question) {
                    ...
                    if (selected == null) {
                        ...
                        vm.model.checkValue[question.id] = true;
                    } else if (selected != null && vm.model.selectedValue[selected.questionId]) {
                        vm.model.checkValue[question.id] = false;
        ...
        templateUrl: '~/App/tenant/views/assessment/painAssessment/ps-detail-view.cshtml'
    });
})();