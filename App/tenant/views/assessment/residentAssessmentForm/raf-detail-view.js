(function () {
    appModule.component('rafDetailView', {
        bindings: {
            title: '<',
            mode: '<',
            template: '<',
        ...
        controllerAs: 'vm',
        controller: [
            '$scope', '$uibModal', '$timeout', 'commonFormFunction', 'abp.services.app.commonLookup', '$stateParams', '$state', '$anchorScroll', '$location', 'abp.services.app.user', '$filter', '$rootScope', 'abp.services.app.residentAssessmentForm', 'abp.services.app.assessment', 'stateNavigation',
            function ($scope, $uibModal, $timeout, commonFormFunction, commonLookupService, $stateParams, $state, $anchorScroll, $location, userService, $filter, $rootScope, residentAssessmentFormService, assessmentService, stateNavigation) {
                ...
                vm.$doCheck = function () {
                    ...
                    if (!angular.equals(vm.preAssessmentName, vm.model.actualAssessment.assessmentName) && vm.model.actualAssessment.assessmentName != undefined) {                        
                        assessmentService.getCategories(vm.model.actualAssessment.assessmentName).then(function (result) {
                ...
                vm.load = function () {
                    ...
                    if ($stateParams.id) {
                        ...
                        vm.mode = 'view';
                        residentAssessmentFormService.getPersonAssessmentById(vm.actualAssessmentId).then(function (result) {
                        ...
                        residentAssessmentFormService.getPersonAssessmentById(vm.actualAssessmentId).then(function (result) {
                            ...
                            if ($stateParams.template == 'person') {
                                ...
                                vm.model.actualAssessment.sourceOfRAF = "Post-Admission";
                            }
                            ...
                            if ($stateParams.template == 'referral') {
                                ...
                                vm.model.actualAssessment.sourceOfRAF = "Pre-Admission";
                            }
                ...
                vm.save = function (closed) {
                    ...
                    convertAnswer();
                    if ($stateParams.id) {
                        residentAssessmentFormService.updatePersonAssessment(vm.model.actualAssessment).then(function () {
                        ...
                        residentAssessmentFormService.createPersonAssessment(vm.model.actualAssessment).then(function (result) {
                            abp.notify.info(app.localize('SavedSuccessfully'));
                            if (closed) {
                                vm.close();
                            } else {
                                if (typeof $stateParams.personId != 'undefined') {
                                    $state.go('tenant.rafDetailPersonMode', { assessment: $stateParams.assessment, id: result.data.id, personId: $stateParams.personId });
                                } else {
                                    $state.go('tenant.rafDetail', { assessment: $stateParams.assessment, id: result.data.id });
                                }
                        ...
                        abp.message.confirm(
                            ...
                            function (isConfirmed) {
                                if (isConfirmed) {
                                    convertAnswer(true);
                                    residentAssessmentFormService.updatePersonAssessment(vm.model.actualAssessment).then(function () {
                                        residentAssessmentFormService.confirm(vm.actualAssessmentId).then(function (result) {
                                            vm.load();
                ...
                vm.actualClone = function (title, message) {
                    abp.message.confirm(
                        ...
                        function (isCloned) {
                            if (isCloned) {
                                residentAssessmentFormService.clone(vm.model.actualAssessment.id).then(function (result) {
                                    if (typeof $stateParams.personId != 'undefined' || typeof $stateParams.referralId != 'undefined') {
                                        $state.go('tenant.rafDetailPersonMode', { template: $stateParams.template, description: $stateParams.description, assessment: $stateParams.assessment, id: result.data.id, referralId: $stateParams.referralId, personId: $stateParams.personId });
                                    } else {
                                        $state.go('tenant.rafDetail', { template: $stateParams.template, description: $stateParams.description, assessment: $stateParams.assessment, id: result.data.id });
                                    }
                                }).finally(function () {
                                    vm.saving = false;
                                });
                ...
                vm.delete = function () {
                    ...
                    abp.message.confirm(
                        ...
                        function (isConfirmed) {
                            if (isConfirmed) {
                                residentAssessmentFormService.deletePersonAssessment(
        ...
        templateUrl: '~/App/tenant/views/assessment/residentAssessmentForm/raf-detail-view.cshtml'
    });
})();