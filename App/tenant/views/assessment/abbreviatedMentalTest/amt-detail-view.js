(function () {
    appModule.component('amtDetailView', {
        bindings: {
            title: '<',
            mode: '<',
            template: '<',
                        ...
                        assessmentService.createPersonAssessment(vm.model.actualAssessment).then(function (result) {
                            ...
                            if (closed) {
                                ...
                                if (typeof $stateParams.personId != 'undefined') {
                                    $state.go('tenant.amtDetailPersonMode', { assessment: $stateParams.assessment, id: result.data.id, personId: $stateParams.personId });
                                } else {
                                    $state.go('tenant.amtDetail', { assessment: $stateParams.assessment, id: result.data.id });
                                }
        ...
        templateUrl: '~/App/tenant/views/assessment/abbreviatedMentalTest/amt-detail-view.cshtml'
    });
})();