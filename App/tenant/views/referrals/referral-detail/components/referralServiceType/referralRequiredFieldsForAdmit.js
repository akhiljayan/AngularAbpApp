(function () {
    appModule.controller('tenant.views.referrals.referralRequiredFieldsForAdmit', [
        '$scope', '$state', '$timeout', '$uibModal', '$uibModalInstance', 'abp.services.app.masterLookUp', 'referralValidationObj', 'abp.services.app.referral', 'abp.services.app.serviceType',
        function ($scope, $state, $timeout, $uibModal, $uibModalInstance, masterLookup, referralValidationObj, appReferralService, serviceType) {
            var vm = this;
            vm.saving = false;
            ...
            vm.save = function () {
                ...
                appReferralService.saveRequiredReferralFieldsForAdmission(vm.referral).then(function (result) {
                    if (result.data) {
                        ...
                        $state.go('tenant.admissionNew', { action: admAction, referralId: vm.referral.referralServiceType.referral.id, id: vm.referral.referralServiceType.id });
